...
import { SteamGameInfo, IGDBGame, Game } from '../../models/game.model';
import { GameService } from '../../services/game.service';
import { MatSnackBar } from '@angular/material/snack-bar';
import { FADE_IN_UP, CARD_ANIMATION } from '../../utils/animations';

interface SteamGameWithIGDB extends SteamGameInfo {
  igdbData?: IGDBGame;
  inBacklog?: boolean;
...
export class SteamLibraryComponent implements OnInit, OnDestroy {
  steamGames: SteamGameWithIGDB[] = [];
  filteredGames: SteamGameWithIGDB[] = [];
  displayedGames: SteamGameWithIGDB[] = [];
  backlogGames: Game[] = [];
  ...
  loadData(): void {
    ...
    forkJoin({
      ...
      backlogGames: this.gameService.getAllGames()
    }).pipe(takeUntil(this.destroy$)).subscribe({
      next: ({ steamLibrary, backlogGames }) => {
        this.backlogGames = backlogGames.data || [];
        
        // Create a map of steam app IDs to backlog games
        const steamAppIdMap = new Map<string, Game>();
        this.backlogGames.forEach(game => {
          if (game.steamAppId) {
            steamAppIdMap.set(game.steamAppId, game);
          }
        });
        
        // Enrich steam games with backlog info
        this.steamGames = (steamLibrary.data || []).map(game => ({
          ...game,
          inBacklog: steamAppIdMap.has(game.appId),
          backlogGameId: steamAppIdMap.get(game.appId)?.id
        }));
  ...
  importToBacklog(game: SteamGameWithIGDB): void {
  ...
  private createBasicGame(game: SteamGameWithIGDB): void {
  ...
  viewInBacklog(game: SteamGameWithIGDB): void {
  ...
  getGameIcon(game: SteamGameWithIGDB): string {
    if (game.imgIconUrl) {
  ...
  trackByAppId(index: number, game: SteamGameWithIGDB): string {
    return game.appId;
  }
